...
<div class="max-w-7xl mx-auto px-6 py-16">
    <div class="text-center mb-16">
        ...
        {{-- Search form --}}
        <form id="searchForm" class="max-w-2xl mx-auto" action="{{ route('search') }}" method="POST" role="search" aria-label="Search">
            @csrf
            <div class="flex flex-col sm:flex-row items-center bg-gray-900/50 backdrop-blur-sm rounded-full border border-blue-500/30 p-2 hover:border-blue-500/60 transition-colors">
                <label for="searchInput" class="sr-only">Search by player tag or clan tag/name</label>
                <input 
                    id="searchInput"
                    type="text" 
                    name="tag"
                    ...
                    required
                >
                <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white px-8 py-3 rounded-full transition-colors flex items-center space-x-2">
    ...
    {{-- Loader Overlay --}}
    <div id="loaderOverlay" class="fixed inset-0 flex items-center justify-center bg-black/50 z-50 hidden" aria-hidden="true">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        Object.entries(buttons).forEach(([type, btn]) => {
            ...
            btn.addEventListener('click', () => {
                if (searchTypeInput) searchTypeInput.value = type;
                ...
                Object.values(buttons).forEach(b => b && b.classList.remove('active-tab'));
